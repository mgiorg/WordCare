...
document.addEventListener('DOMContentLoaded', function() {
    ...
    gsap.utils.toArray('.service-card').forEach((card, i) => {
        ScrollTrigger.create({
            trigger: card,
            start: 'top 80%',
            toggleClass: 'animate__animated animate__fadeInUp',
            once: true,
            delay: i * 0.2
        });
    });
    ...
    gsap.utils.toArray('.feature').forEach((feature, i) => {
        ScrollTrigger.create({
            trigger: feature,
            start: 'top 80%',
            toggleClass: 'animate__animated animate__fadeIn',
            once: true,
            delay: i * 0.2
        });
    });
    ...
    window.addEventListener('scroll', function() {
        ...
        if (scrollY <= heroSection.offsetHeight) {
            ...
            document.querySelector('.hero-image').style.transform = `translateY(${translateY}px)`;
            document.querySelector('.hero-content').style.transform = `translateY(${translateY * 0.5}px)`;
        }
    });
